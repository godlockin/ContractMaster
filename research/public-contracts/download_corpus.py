...
def main():
    ...
    # Keep successful raw downloads immutable; P08 is reprocessed to mask its public watermark name.
    todo=[c for c in CASES if c[0] not in prior or prior[c[0]]['download_status']=='FAILED' or c[0] in {'P05','P08'}]
    with ThreadPoolExecutor(max_workers=4) as pool: fresh={r['id']:r for r in pool.map(fetch,todo)}
    results=[fresh.get(c[0],prior.get(c[0])) for c in CASES]
